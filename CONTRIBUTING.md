...
1. **类型安全**：使用 `mypy --strict sciflow/` 检查，零容忍错误
2. **代码风格**：使用 `ruff check sciflow/ tests/ scripts/`，零容忍错误
3. **依赖方向**：`sciflow/runtime/` 禁止 import `sciflow/agent/` 或 LLM SDK。用 `python scripts/check_deps.py` 验证
...
```bash
# 单元+集成
pytest tests/unit/ tests/integration/ --cov=sciflow

# 端到端（4 个 Demo）
pytest tests/e2e/ -v

# 全部
pytest tests/ --cov=sciflow
```
